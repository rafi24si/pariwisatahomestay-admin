...
<form action="{{ route('homestay.update', $homestay->homestay_id) }}" method="POST">
    ...
    <select name="pemilik_warga_id" required>
    ...
    <input type="number" step="0.01" name="harga_per_malam"
           value="{{ old('harga_per_malam', $homestay->harga_per_malam) }}" required><br>

    <label for="status">Status:</label>
    <input type="text" name="status"
           value="{{ old('status', $homestay->status) }}" required><br>
